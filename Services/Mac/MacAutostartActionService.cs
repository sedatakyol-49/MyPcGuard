...
public sealed class MacAutostartActionService : IAutostartActionService
{
    private static readonly string Home = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
    private static readonly string AppDataDirectory = Path.Combine(Home, "Library", "Application Support", "MyPcGuard");
    ...
    public async Task<StartupActionResult> EnableAsync(StartupItem item, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return await CompleteAsync("Enable", item, StartupActionResult.Fail("Kein Backup fuer diesen LaunchAgent gefunden."), cancellationToken);
            }
            ...
            return await CompleteAsync("Enable", item, StartupActionResult.Ok("LaunchAgent wurde aktiviert.", item with { IsEnabled = true, CanEnable = false, CanDisable = item.StartupClassification != StartupClassification.Essential }), cancellationToken);
        }
    ...
    public async Task<StartupActionResult> DisableAsync(StartupItem item, CancellationToken cancellationToken)
    {
        ...
        {
            return await CompleteAsync("Disable", item, StartupActionResult.Fail("Systemkritische LaunchAgents koennen nicht deaktiviert werden."), cancellationToken);
        }
        ...
        try
        {
            ...
            {
                return await CompleteAsync("Disable", item, StartupActionResult.Fail("LaunchAgent Datei wurde nicht gefunden."), cancellationToken);
            }

            Directory.CreateDirectory(DisabledDirectory);
            var backupFile = Path.Combine(DisabledDirectory, Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(item.Id)).Replace('/', '_') + ".plist");
            ...
            if (existing is null)
            {
                backups.Add(new MacStartupBackupEntry
                {
                    Id = item.Id,
            ...
            await WriteBackupsAsync(backups, cancellationToken);
            return await CompleteAsync("Disable", item, StartupActionResult.Ok("LaunchAgent wurde deaktiviert. Scan erneut starten empfohlen.", item with { IsEnabled = false, CanEnable = true, CanDisable = false }), cancellationToken);
        }
        catch (Exception ex) when (IsAccessDenied(ex))
        {
    ...
    public async Task<StartupActionResult> OpenFileLocationAsync(StartupItem item, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return await CompleteAsync("OpenFileLocation", item, StartupActionResult.Ok("Finder wurde geoeffnet."), cancellationToken);
        }
        catch (Exception ex)
        {
            return await CompleteAsync("OpenFileLocation", item, StartupActionResult.Fail($"Finder konnte nicht geoeffnet werden: {ex.Message}"), cancellationToken);
        }
    }
    ...
    private static async Task<List<MacStartupBackupEntry>> ReadBackupsAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            return await JsonSerializer.DeserializeAsync<List<MacStartupBackupEntry>>(stream, cancellationToken: cancellationToken) ?? [];
        }
        catch
        {
            return [];
    ...
    private static async Task WriteBackupsAsync(List<MacStartupBackupEntry> backups, CancellationToken cancellationToken)
    {
